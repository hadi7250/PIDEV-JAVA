...
public class UserService implements IUserService {
    ...
    public void add(User user) throws SQLException {
        applyAdminFormDefaults(user);
        register(user);
    }

    @Override
    public void update(User user) throws SQLException {
        syncRoleFromRolesJson(user);
        updateProfile(user);
    }
    ...
    private static void syncRoleFromRolesJson(User user) {
        String json = user.getRoles();
        if (json != null && json.toUpperCase().contains("ADMIN")) {
            user.setRole("ADMIN");
    ...
    private static void applyAdminFormDefaults(User user) {
        syncRoleFromRolesJson(user);
        if (user.getAge() <= 0) {
            user.setAge(18);
        }
