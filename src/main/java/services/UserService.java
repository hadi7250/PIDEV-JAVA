...
public class UserService {
    ...
    public boolean register(User user) {
        String query = "INSERT INTO user (first_name, last_name, age, email, password, role) VALUES (?, ?, ?, ?, ?, ?)";
    ...
    public boolean updateProfile(User user) {
        String query = "UPDATE user SET first_name = ?, last_name = ?, age = ?, email = ?, password = ? WHERE id = ?";
        try (PreparedStatement pstmt = cnx.prepareStatement(query)) {
            pstmt.setString(1, user.getFirstName());
            pstmt.setString(2, user.getLastName());
            pstmt.setInt(3, user.getAge());
    ...
    public boolean updateProfileWithPhoto(User user, String photoPath) {
        String query = "UPDATE user SET first_name = ?, last_name = ?, age = ?, email = ?, password = ?, photo_path = ? WHERE id = ?";
        try (PreparedStatement pstmt = cnx.prepareStatement(query)) {
            pstmt.setString(1, user.getFirstName());
            pstmt.setString(2, user.getLastName());
            pstmt.setInt(3, user.getAge());
