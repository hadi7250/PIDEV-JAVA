    ...
    private void navigateToDashboard(User user) {
        try {
            ...
            if (user.isAdmin()) {
                ...
                // Regular user → événements (fx:controller dans le FXML ; user id via SessionManager dans initialize)
                FXMLLoader loader = new FXMLLoader(getClass().getResource("/fxml/Event/front_events.fxml"));
                Parent root = loader.load();
                Stage stage = (Stage) emailField.getScene().getWindow();
                MainFx.setPrimaryStage(stage);
                stage.setScene(new Scene(root));
                stage.setTitle("EduConnect — Événements");
                stage.setMaximized(true);
                stage.show();
            }
