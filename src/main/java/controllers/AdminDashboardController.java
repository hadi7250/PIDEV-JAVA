...
public class AdminDashboardController implements Initializable {
    ...
    @FXML private void goHome()             { Main.switchScene("/fxml/main_app.fxml"); }
    ...
    private void loadUsersTable() {
        try {
            ...
            colName.setCellValueFactory(cd -> new SimpleStringProperty(cd.getValue().getFirstName() + " " + cd.getValue().getLastName()));
            ...
            bindLiveTableFilter(searchUs, filteredUs, u ->
                    String.join(" ", nz(u.getEmail()), nz(u.getFirstName() + " " + u.getLastName()), nz(u.getStatus())));
    ...
    private void openUserForm(User existing) {
        ...
        TextField tfName = new TextField(isEdit ? existing.getFirstName() + " " + existing.getLastName() : "");
        ...
        TextField tfNsc = new TextField(""); // NSC not supported
        ...
        TextField tfRoles = new TextField(isEdit ? "[\"" + existing.getRole() + "\"]" : "[\"ROLE_USER\"]");
        ...
        dialog.setResultConverter(btn -> {
            ...
            User u = isEdit ? existing : new entities.User("", "", 0, tfEmail.getText().trim(), "", "USER");
            u.setEmail(tfEmail.getText().trim());
            u.setFirstName(tfName.getText().trim().contains(" ") ? tfName.getText().trim().split(" ", 2)[0] : tfName.getText().trim()); u.setLastName(tfName.getText().trim().contains(" ") ? tfName.getText().trim().split(" ", 2)[1] : "");
    ...
    private java.util.List<String> userRow(User u) {
        return java.util.List.of(nz(u.getEmail()), nz(u.getFirstName() + " " + u.getLastName()), nz(u.getStatus()));
    }
    ...
    private void generateCertificatePdf(Certificat c) {
        try {
            ...
            try {
                entities.User u = userService.getById(c.getUserId());
                if (u != null) {
                    String full = (u.getFirstName() + " " + u.getLastName()).trim();
                    if (!full.isEmpty()) userName = full;
                }
                models.Event e = eventService.getById(c.getEventId());
                if (e != null && e.getTitre() != null && !e.getTitre().trim().isEmpty()) eventTitle = e.getTitre();
