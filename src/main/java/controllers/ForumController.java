...
public class ForumController {
    ...
    private int getUserIdByName(String fullName) {
        ...
        String[] queries = {
            "SELECT id FROM user WHERE CONCAT(firstName, ' ', lastName) = ?",
            "SELECT id FROM user WHERE email = ?",
            "SELECT id FROM user WHERE firstName = ? OR lastName = ? OR email = ?"
        };
        ...
        try (Connection conn = MyDB.getInstance().getConnection()) {
            ...
            for (int i = 0; i < queries.length; i++) {
                ...
                try (PreparedStatement stmt = conn.prepareStatement(queries[i])) {
                    ...
                    if (i == 2) { // The multi-field query (last index is now 2 instead of 3)
