...
public class FrontEventsController implements Initializable {
    ...
    // private static final int CURRENT_USER_ID = 1; // Removed hardcoded ID
    ...
    private void goHome() {
        ...
        if (user != null && user.isAdmin()) {
            Main.switchScene("/fxml/AdminHomeDashboard.fxml");
        } else {
            Main.switchScene("/fxml/UserHomeDashboard.fxml");
        }
    }
    ...
    private void showEventDetailsPage(Event ev) {
        ...
        int joinedCount = 0;
        try {
            hasJoined = participationService.userHasJoined(utils.SessionManager.getInstance().getCurrentUserId(), ev.getId());
            hasRated = ratingService.userHasRated(utils.SessionManager.getInstance().getCurrentUserId(), ev.getId());
        ...
        joinBtn.setOnAction(e -> {
            try {
                ...
                int currentUserId = utils.SessionManager.getInstance().getCurrentUserId();
    ...
    private void openRatingDialog(Event ev) {
        try {
            if (ratingService.userHasRated(utils.SessionManager.getInstance().getCurrentUserId(), ev.getId())) {
            ...
            dialog.setResultConverter(btn -> {
                if (btn == submitBtn) {
                    Rating r = new Rating();
                    r.setUserId(utils.SessionManager.getInstance().getCurrentUserId());
    ...
    private void loadMyReviews() {
        ...
        try {
            ...
            List<Rating> mine = all.stream().filter(r -> r.getUserId() == utils.SessionManager.getInstance().getCurrentUserId()).toList();
    ...
    private void loadMyCertificates() {
        ...
        card.setPadding(new Insets(18));
        try {
            List<Certificat> certs = certificatService.getByUser(utils.SessionManager.getInstance().getCurrentUserId());
